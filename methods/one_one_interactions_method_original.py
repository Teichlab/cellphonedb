...
np.random.seed(1)  #####   setting the random seed so that we get always same random shuffles
...
how_many = sys.argv[
    2]  ##### on how many pairs should it run, starting from interaction pair on position "num_interactions"
...
counts_filtered = counts.loc[counts.index.isin(
    genes_unique)]  ######   filter the count table so that it contains only genes (rows) that are in the interaction pairs list
...
all_pairs_means = collections.defaultdict(
    dict)  #####   here we will save all the means from the 1000 shufflings, and we will use these 1000 values
