...
class QueryBase(Resource):
    ...
    def _attach_json(self, json_to_send):
        ...
        attachment.set_payload(json.dumps(json_to_send))
...
class CellToCluster(QueryBase):
    def post(self):
        ...
        self._attach_json({'status': 'nice'})
        self._attach_csv(result_df.to_csv(sep='\t'), 'cluster_counts.csv')
...
class ReceptorLigandsInteractions(QueryBase):
    def post(self):
        cells_to_clusters_file = pd.read_table(request.files['cell_to_clusters_file'].stream, index_col=0)
        ...
        self._attach_json({'status': 'nice'})
        self._attach_csv(result_interactions.to_csv(sep='\t'), 'result_interactions.csv')
        self._attach_csv(result_interactions_extended.to_csv(sep='\t'), 'result_interactions_extended.csv')
