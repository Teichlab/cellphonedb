...
class AppConfig():
    ...
    def _load_config(self) -> dict:
        ...
        if self.config_parameters['load_defaults']:
            config = self._load_yaml('cellcommdb/config/{}.yml'.format('base_config'))
        if self.config_parameters['environment']:
            custom_config = self._load_yaml('cellcommdb/config/{}.yml'.format(self.config_parameters['environment']))
