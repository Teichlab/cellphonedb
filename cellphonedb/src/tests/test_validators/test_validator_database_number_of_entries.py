...
class TestValidatorDatabaseNumberOfEntries(CellphoneFlaskTestCase):
    ...
    def test_protein(self):
        ...
        self.assertEqual(884, len(proteins), 'Number of Protein entries are different')
    ...
    def test_multidata(self):
        multidatas = cellphonedb_app.cellphonedb.database_manager.get_repository(
            'multidata').get_all()
        self.assertEqual(996, len(multidatas), 'Number of Multidata entries are different')
    ...
    def test_interaction(self):
        ...
        self.assertEqual(1167, len(interactions), 'Number of interactions are not equal')
    ...
    def test_interaction_cpdb_interactor(self):
        ...
        self.assertEqual(1167, len(interactions[interactions['is_cellphonedb_interactor']]),
                         'of cellphonedb interaction interactors different that expected')

    def test_interaction_guidetopharmacology(self):
        interactions = cellphonedb_app.cellphonedb.database_manager.get_repository(
            'interaction').get_all()
        self.assertEqual(154, len(interactions[interactions['source'] == 'guidetopharmacology.org']),
